...
func ready(t *testing.T, h *testutil.Harness) {
	t.Helper()
	if e := h.Await(2 * time.Second); e.Event != rlm.KindReady || e.Protocol != 3 {
...
func TestReadyHandshakeAndExecute(t *testing.T) {
	...
	if e, _ := h.WantDone("c1", 2*time.Second); e.Status != rlm.StatusOK {
...
func TestInterruptCancelsRunningCell(t *testing.T) {
	...
	for !(gotErr && gotDone) {
		...
		case e := <-h.Events:
			...
			case e.Event == rlm.KindError && e.EName == rlm.EnameKeyboard:
				gotErr = true
			case e.Event == rlm.KindDone && e.ID != nil && *e.ID == "slow" && e.Status == rlm.StatusError:
				gotDone = true
			}
		case <-deadline:
	...
	if e, _ := h.WantDone("after", 2*time.Second); e.Status != rlm.StatusOK {
	...
	if e, _ := h.WantDone("parked", 2*time.Second); e.Status != rlm.StatusError {
...
func TestBackgroundGoroutineOutlivesCell(t *testing.T) {
	...
	if e, _ := h.WantDone("bgcell", 2*time.Second); e.Status != rlm.StatusOK {
	...
	for {
		select {
		case e := <-h.Events:
			if e.Event == rlm.KindStdout && e.ID != nil && *e.ID == "bgcell" &&
...
func TestSnapshotRestore(t *testing.T) {
	...
	if e.Status != rlm.StatusOK || !contains(e.Saved, "x") || !contains(e.Saved, "y") || e.Bytes <= 0 {
		t.Fatalf("snapshot done: %+v", e)
	}

	h.SendReq(map[string]any{"type": "restore", "id": "rest", "path": dir + "/payload.json"})
	if e, _ := h.WantDone("rest", 2*time.Second); e.Status != rlm.StatusOK || !contains(e.Restored, "x") || !contains(e.Restored, "y") {
		t.Fatalf("restore done: %+v", e)
	}

	h.SendReq(map[string]any{"type": "restore", "id": "miss", "path": dir + "/nope.json"})
	if e, _ := h.WantDone("miss", 2*time.Second); e.Status != rlm.StatusOK || e.Reason != "snapshot not found" {
...
func TestAckPythonBootstrap(t *testing.T) {
	...
	h := testutil.NewHarness(t, func(c *rlm.Config) {
		c.Eval = ev
	...
	if e, _ := h.WantDone("boot", 2*time.Second); e.Status != rlm.StatusOK {
	...
	if e, _ := h.WantDone("c", 2*time.Second); e.Status != rlm.StatusOK {
...
func (c *countingEvaluator) Run(env rlm.Env) (rlm.Result, error) {
	c.n.Add(1)
	return rlm.Run(env)
}
