...
- [x] Stock-host integration verified via the go-kernel extension (the
      supported path): `go` tool + ipython interception, glm-5.2 round-trip
      with persistent state. Legacy dev recipe: `PRIME_AGENT_KERNEL_PYTHON`
      shim + `GORLM_ACK_PYTHON_BOOTSTRAP=1`.
